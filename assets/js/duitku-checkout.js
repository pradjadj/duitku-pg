jQuery(document).ready(function($) {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    // Show QRIS payment UI if payment method is QRIS
                    if (response.data.payment_method && response.data.payment_method.startsWith('QR')) {
                        showQRISPayment(response.data);
                    }
                }
            },
            error: function() {
    ...
    // Show QRIS payment UI
    function showQRISPayment(data) {
        var container = $('.duitku-qris-payment-info');
        if (container.length === 0) {
            container = $('<div class="duitku-qris-payment-info"></div>');
            $('.duitku-payment-info').append(container);
        }
        container.empty();

        var html = '<p>Silahkan Scan QRIS berikut ini</p>' +
                   '<div id="qrcode"></div>' +
                   '<p>Nominal Pembayaran: ' + (data.amount ? data.amount : '') + '</p>' +
                   '<p>Batas waktu pembayaran: ' + (data.expiry_time ? data.expiry_time : '') + '</p>';
