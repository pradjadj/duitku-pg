    ...
    protected function display_payment_instructions($order, $va_number, $expiry) {
        ...
        if ($order->has_status('completed')) {
            ...
            if ($this->payment_code === 'FT' || $this->payment_code === 'IR') {
                ...
                echo '<div id="qrcode" style="margin: 10px 0;"></div>';
                // Generate QR code from qrString meta
                $qr_string = $order->get_meta('_qr_string');
                if ($qr_string) {
                    echo '<script type="text/javascript">';
                    echo 'document.addEventListener("DOMContentLoaded", function() {';
                    echo '  new QRCode(document.getElementById("qrcode"), {';
                    echo '    text: ' . json_encode($qr_string) . ',';
                    echo '    width: 250,';
                    echo '    height: 250,';
